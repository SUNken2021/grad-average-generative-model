...
def get_anneal_size(anneal_times, image_size, anneal_prefer='normal'):
    anneal_sizes = []
    if anneal_prefer == 'normal':
        for anneal_time in range(anneal_times + 1):
            anneal_size = np.int(image_size / (2 ** anneal_time))
            anneal_sizes.append(anneal_size)
    elif anneal_prefer == 'detail':
        for anneal_time in range(anneal_times + 1):
            anneal_size = np.int(image_size / (2 ** anneal_time))
            times = np.int(2 ** anneal_time)
            for i in range(times):
                anneal_sizes.append(anneal_size)
    elif anneal_prefer == 'rough':
        for anneal_time in range(anneal_times + 1):
            anneal_size = np.int(image_size / (2 ** anneal_time))
            times = anneal_size
            for i in range(times):
...
class NCSNRunner():
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                X = data_transform(self.config, X)  # 对CIFAR10没有任何影响
    ...
    def train_v1(self):
        ...
        # 生成一个递减至0的数列alphas，作为分块大小的幂指数序列
        # 例如一个batch共32=2^5组数据，那么生成alphas=[5,4,3,2,1,0]
        # 对应分块大小为[32,16,8,4,2,1]
        m_ = np.int(np.log2(self.config.training.batch_size))
        alphas = np.arange(m_,-1,-1)

        """
        目前的问题就是当分块较小时,alpha<=2时会出现梯度爆炸问题
        """

        # 之前的想法是按照一个block对应一个alpha的训练方法
        # 将训练总次数n_epochs平均分为m_+1块，对应递减的alphas序列
        epoch_block_size = self.config.training.n_epochs // (m_ + 1)

        for epoch in range(start_epoch, self.config.training.n_epochs):
            # 每一轮训练重新打乱一次数据的分批
            # 之所以有这个想法，是因为我们取的分块是近似的，使用每个batch中的数据进行二等分、四等分等等的分块
            # 如果在每个epoch当中打乱batch的分配，或许就可以近似的分块更逼近实际的分块情况
            dataloader = DataLoader(dataset, batch_size=self.config.training.batch_size, shuffle=True,
                                    num_workers=self.config.data.num_workers)
            ...
            for i, (X, y) in enumerate(dataloader):
                logging.info("dataloader{}".format(i))
                score.train()
                step += 1
                ...
                # 用当前epoch的编号整除block_size来确定alpha在alphas中的编号
                alpha_index = epoch // epoch_block_size
                alpha=alphas[alpha_index]
                # alpha=2

                loss = score_matching_estimator(score, X, alpha, sigmas)

                tb_logger.add_scalar('loss', loss, global_step=step)
                tb_hook()


                logging.info("step: {}, alpha:{}/m':{}, loss: {}".format(step, alpha, m_, loss.item()))
                ...
                if step % 100 == 0:
                    ...
                    with torch.no_grad():
                        test_dsm_loss = score_matching_estimator(test_score, test_X, alphas[-1], sigmas)
                        tb_logger.add_scalar('test_loss', test_dsm_loss, global_step=step)
                        test_tb_hook()
                        logging.info("step: {}, test_loss: {}".format(step, test_dsm_loss.item()))
